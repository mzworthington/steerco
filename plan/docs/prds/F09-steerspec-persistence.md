...
- [x] **IndexedDB directory handles** - store `FileSystemDirectoryHandle` (and file name) so folder Save survives tab refresh without re-open; re-prompt for permission when the browser requires it. Implemented via `workspaceDirectoryStore` (IndexedDB `steerco-workspace`).
