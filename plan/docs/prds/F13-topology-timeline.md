...
**Slice:** 1.5 (schema) → 3 (timeline view)  
**Frameworks:** Team Topologies (shape evolves; collaboration is time-boxed; cognitive load changes with capacity and interaction modes) — see [OPERATING_MODEL_ALIGNMENT.md](../OPERATING_MODEL_ALIGNMENT.md) · [key concepts](https://teamtopologies.com/key-concepts)

## Problem

Leaders see today’s topology snapshot but cannot answer: _when did capacity grow or shrink, when did interactions change, and did load get better or worse?_ Without a timeline, stop/start decisions lack the “before / after” evidence that Team Topologies assumes when shape evolves.

## Goal

Record and visualise **capacity changes** (members / FTE%) and **relationship changes** (mode start/end, including Collaboration and Facilitation windows) on a shared timeline so directors can scrub steering periods and see topology intent over time.
...
2. Lanes (or swimlanes) per team type zone or per team; markers for capacity deltas and relationship spans.
...
5. Scrubber / date picker: org shape + mismatches recompute **as of** selected date.
...
- Timeline view shows both capacity deltas and relationship spans; as-of date changes the projected org shape
