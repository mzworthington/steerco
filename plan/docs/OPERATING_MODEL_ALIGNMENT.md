...
Matthew Skelton, Manuel Pais (IT Revolution). Subtitle: *Organizing Business and Technology for Fast Flow of Value*. ISBN 9781966280002. Published 23 Sep 2025.
...
| Stream-aligned ≈ customer-facing | `team.role: customer_facing` |
| Platform | `team.role: shared_platform` |
| Enabling ≈ coaching/support | `team.role: coaching_support` |
...
| Intent over static org chart | Topology *intent* in SteerSpec |
...
| --- | --- |
| **Clearly articulated intent and purpose** | Topology and bets are *intent* artifacts leaders evolve—not a frozen org chart export |
...
| **Platform = platform grouping** | A “platform” may be many teams with a shared purpose—not one box. Schema/UI must allow platform *groupings* |
| **Value stream grouping** | Optional grouping of stream-aligned teams around a common value stream / outcome |
| **Fractal design** | Same patterns at multiple zoom levels (e.g. stream-aligned teams *inside* a platform grouping) |
...
| --- | --- | --- |
| Stream-aligned | `customer_facing` | Keep exec label “Customer-facing”; Technical mode / glossary: stream-aligned |
| Platform (grouping) | `shared_platform` | Support **platform grouping** (parent grouping + member teams) |
| Enabling | `coaching_support` | Keep “Coaching / support”; glossary: enabling; Facilitation mode default |
| Complicated subsystem | *Missing* | Add `complicated_subsystem` (exec: “Specialist subsystem”) — rare, high-complexity domain owned by specialists so stream teams stay focused |
...
| Fractal overload | Platform grouping with too many internal members *and* external dependents |
...
- [ ] MoS framing on outcomes page; bet detail shows related outcome MoS as context
- [ ] Decision note helper copy prefers MoS / evidence language
- [ ] F03 / steering copy: cognitive load, fast flow, platform reduces load; overload banner wording
...
- [ ] `team.role` add `complicated_subsystem`
