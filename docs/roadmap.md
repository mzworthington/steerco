...
SteerCo delivers an **investment contract** (EDGE value-driven operating model - Lean Value Tree **and** the teachings beyond it), **problem-space boundaries** (Domain-Driven Design / Eric Evans), and **topology intent** (Team Topologies) in a steering workspace - with a board pack as the shareable export. Rationale, vocabulary bridge, and detailed checklists: [operating-model-alignment.md](./operating-model-alignment.md).
...
- [x] Operating-model alignment (EDGE + Team Topologies 2e + DDD / Eric Evans) - [operating-model-alignment.md](./operating-model-alignment.md)
...
| **EDGE beyond LVT**   | MoS fitness framing; PVR = decision notes + review dates; fundingStance as Integrated Backlog mix cue; capability bets as Tech@Core revitalize signal - Product brief schema deferred to Slice 3+                                                                                                                                       |
...
**PRD impact:** amend F02–F07 + [steerspec.md](./steerspec.md) / schema; new mismatch rules in `@steerco/core`. Prefer additive fields + migrate helpers; bump `apiVersion` only if needed.
...
**Status:** Slice 1.5 complete - schema + mismatches + executive edit surfaces (bet MoS/funding/review, relationship `expectedUntil`, member add/edit, decision `measuredMetricIds`, `topologyEvents[]` in contract). Timeline UI landed in Slice 3 ([F13](./prds/F13-topology-timeline.md)).
...
- [x] Import & merge teams ([F11](./prds/F11-import-merge.md)) - catalog-file path (no OAuth)
- [x] Write-back policy enforced (Backstage Group YAML export and graph contribution supported; other providers blocked)
- [x] Technical mode surfaces ([F12](./prds/F12-technical-mode.md)) - includes EDGE/TT vocabulary bridge for staff+
...
- [x] **Topology timeline deep-dive ([F13](./prds/F13-topology-timeline.md)):** capacity deltas + relationship spans scrubber
...
- Heavy Product brief / requirements documentation
...
| Full rationale                                                                   | [operating-model-alignment.md](./operating-model-alignment.md)                                                                            |
